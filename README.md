...
📁 프로젝트 폴더
 ├─ main.py         # 게임 전체 로직
 ├─ highscore.txt   # 최고 점수 저장 파일 (자동 생성/갱신)
 └─ README.md       # 프로젝트 설명서
```
...
* 인터넷 연결이 없거나 API 응답이 실패하면,
  기본 메시지로 대체되므로 게임 진행에는 문제가 없습니다.
* OS에 따라 한글 폰트 렌더링이 다르게 보일 수 있습니다.
  필요 시 폰트 경로를 환경에 맞게 변경하는 것을 권장합니다.
* 전체 화면 / 해상도 변경, 모바일 포팅 등은 별도 구현이 필요합니다.

---

## 🙌 기여 및 피드백 (Contributing)

* 버그 제보, 기능 제안, 코드 리팩토링 PR 모두 환영합니다.
* 이펙트 추가, 사운드 효과, 새로운 모드(예: 난이도 선택, 아이템 기능) 등
  다양한 아이디어도 자유롭게 이슈로 남겨주세요.

---

원하면:

* 스크린샷 섹션 추가용 Markdown 템플릿
* 영어-only 버전 README
* 더 기술적인 설명(충돌 판정 로직, 애니메이션 구현 방식 등)
