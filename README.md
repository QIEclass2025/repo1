...
업데이트 버전에서는 기존 게임성은 유지하면서, 다음과 같은 요소들이 추가·개선되었습니다.

1. **대각선 도티 미사일 추가**

   * 일정 점수 이상이 되면 화면 좌·우측에서 **대각선으로 날아오는 도티 장애물**이 등장합니다.
   * 발사 전, 이동 경로를 보여주는 **빨간 경고선**이 먼저 그려져, 미리 피할 수 있도록 힌트를 제공합니다.

2. **플레이 중 떠다니는 명언 텍스트**

   * 게임 시작 시 불러온 영어 명언을 여러 줄 텍스트 박스로 만들어,
   * 화면 안에서 천천히 움직이고 벽에 부딪히면 방향을 바꾸는 **배경 애니메이션**으로 활용합니다.
   * 게임 오버 때만이 아니라, **플레이 내내 명언이 화면을 돌아다니는 연출**이 들어갔습니다.

3. **게임 오버 화면 흔들림 효과(Screen Shake)**

   * 캐릭터가 도티와 충돌하면, 잠깐 동안 화면 전체를 흔들어 주는 연출이 추가되었습니다.
   * 단순히 “끄는” 느낌이 아니라, 피격감이 있는 **피니시 이펙트**로 마무리됩니다.

4. **OS별 한글 폰트 자동 선택 로직**

   * Windows / macOS 등 운영체제에 따라 한글 폰트를 자동으로 선택하고,
   * 폰트를 찾지 못하더라도 기본 폰트로 폴백하도록 보완하여 **환경별 글자 깨짐 가능성을 줄였습니다.**

5. **난이도·스폰 로직 세부 조정**

   * 시간 경과에 따른 속도 증가, 스폰 주기 조정 등 난이도曲선을 좀 더 자연스럽게 다듬어
   * 초반에는 진입 장벽을 낮추고, 후반에는 손에 땀을 쥐는 난이도를 느낄 수 있도록 튜닝했습니다.
